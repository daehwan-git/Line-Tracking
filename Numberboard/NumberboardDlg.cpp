...
BEGIN_MESSAGE_MAP(CNumberboardDlg, CDialogEx)
	...
	ON_BN_CLICKED(IDC_BUTTON5, &CNumberboardDlg::OnBnClickedButton5)
	ON_BN_CLICKED(IDC_BUTTON6, &CNumberboardDlg::OnBnClickedButton6)
	ON_BN_CLICKED(IDC_BUTTON7, &CNumberboardDlg::OnBnClickedButton7)
	ON_BN_CLICKED(IDC_BUTTON8, &CNumberboardDlg::OnBnClickedButton8)
	ON_BN_CLICKED(IDC_BUTTON9, &CNumberboardDlg::OnBnClickedButton9)
	ON_BN_CLICKED(IDC_BUTTON10, &CNumberboardDlg::OnBnClickedButton10)
	ON_BN_CLICKED(IDC_BUTTON11, &CNumberboardDlg::OnBnClickedButton11)
	ON_BN_CLICKED(IDC_BUTTON12, &CNumberboardDlg::OnBnClickedButton12)
	ON_BN_CLICKED(IDC_BUTTON13, &CNumberboardDlg::OnBnClickedButton13)
	ON_BN_CLICKED(IDC_BUTTON14, &CNumberboardDlg::OnBnClickedButton14)
	ON_BN_CLICKED(IDC_BUTTON15, &CNumberboardDlg::OnBnClickedButton15)
...
void CNumberboardDlg::OnBnClickedButton5()
{
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.

	Canny(m_srcImage, m_dstImage, 50, 200);

	Blob(m_dstImage, m_dstImage2);
}


void CNumberboardDlg::OnBnClickedButton6()
{
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
	...
	filter2D(m_srcImage, prewitt1, -1, dst1);
	filter2D(m_srcImage, prewitt2, -1, dst2);

	Mat absX, absY;
	convertScaleAbs(prewitt1, absX);
	convertScaleAbs(prewitt2, absY);

	addWeighted(absX, 1.0, absY, 0.5, 0, m_dstImage);
	Mat m_dstImage_backup;
	m_dstImage.copyTo(m_dstImage_backup);
	Mat dst;
	Laplacian(m_srcImage, dst, CV_16S, 3, 1, 0);
	convertScaleAbs(dst, m_dstImage);
	addWeighted(m_dstImage_backup, 0.5, m_dstImage, 0.5, 0, m_dstImage);

	Blob(m_dstImage, m_dstImage2);
}


void CNumberboardDlg::OnBnClickedButton7()
{
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
	
	Mat blur, gray, dst, abs_dst;
	GaussianBlur(m_srcImage, blur, Size(3, 3), 10);
...
void CNumberboardDlg::OnBnClickedButton8()
{
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
		...
		Mat dst;
		filter2D(m_srcImage, m_dstImage, -1, emboss, Point(-1, -1), 128);
...
void CNumberboardDlg::OnBnClickedButton9()
{
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
		...
		Mat blurred;
		GaussianBlur(m_srcImage, blurred, Size(), 0);

		float alpha = 1.f;
		Mat m_dstImage = (1 + alpha) * m_srcImage - alpha * blurred;

		String desc = format("sigam : %d", 0);
		putText(m_dstImage, desc, Point(10, 30), FONT_HERSHEY_SIMPLEX, 1.0, Scalar(255), 1, LINE_AA);

		DrawImage2(m_dstImage);
...
void CNumberboardDlg::OnBnClickedButton10()
{
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
	

	GaussianBlur(m_srcImage, m_dstImage, Size(5, 5), 3);
...
void CNumberboardDlg::OnBnClickedButton11()
{
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
	

		for (int stddev = 10; stddev <= 50; stddev += 10) {
			Mat noise(m_srcImage.size(), CV_32SC1);
			randn(noise, 0, stddev);

			Mat dst;
			add(m_srcImage, noise, m_dstImage, Mat(), CV_8U);

			String text = format("stddev: %d", stddev);
			putText(m_dstImage, text, Point(10, 30), FONT_HERSHEY_SIMPLEX, 1.0, Scalar(255), 1, LINE_AA);
			cv::imshow(format("stddev: %d", stddev), m_dstImage);
			DrawImage2(m_dstImage2);
		}
		waitKey();
}


void CNumberboardDlg::OnBnClickedButton12()
{
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
	
	cv::bilateralFilter(m_srcImage, m_dstImage, 5, 150, 150);
...
void CNumberboardDlg::OnBnClickedButton13()
{
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
	cv::blur(m_srcImage, m_dstImage, cv::Size(7, 7));
...
void CNumberboardDlg::OnBnClickedButton14()
{
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
	cv::medianBlur(m_srcImage, m_dstImage, 7);
...
void CNumberboardDlg::OnBnClickedButton15()
{
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
	
	boxFilter(m_srcImage, m_dstImage, -1, Size(10, 10));
